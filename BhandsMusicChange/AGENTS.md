...
BhandsMusic 是 Windows Electron 桌面音乐播放器，核心体验包括搜索、播放、歌单、歌词、3D 歌单架、粒子视觉预设、DIY 视觉控制台和 GitHub 自动更新。

- 当前可运行程序：`E:\桌面\播放器软件\BhandsMusic\BhandsMusic.exe`
- 当前运行版主目录：`E:\桌面\播放器软件\BhandsMusic`
...
新对话开始处理 BhandsMusic 前，必须先确认当前目录是：

```powershell
E:\桌面\播放器软件\BhandsMusic\resources\app
```
