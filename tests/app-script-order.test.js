...
test('00-prelude.js 必须排第一（跨 script 提升依赖它最先可用）', () => {
  assert.equal(APP_JS_FILES[0], 'public/js/app/00-prelude.js');
  assert.equal(LISTED[0], 'public/js/app/00-prelude.js',
    'prelude 不在第一位：后面文件的顶层语句会在它之前执行，跨 script 提升的函数仍会 ReferenceError');
  const htmlTagIdx = html.indexOf('src="js/app/00-prelude.js"');
  const firstAppIdx = html.indexOf('src="js/app/01-state.js"');
  assert.ok(htmlTagIdx > 0 && htmlTagIdx < firstAppIdx, 'index.html 里 prelude 必须写在 01-state.js 之前');
});
