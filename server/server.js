 ...
 *
 *  功能模块：
 *  - 网易云音乐：搜索、歌曲URL、封面/音频代理、歌词
 *  - QQ音乐：搜索、歌曲URL、歌词、评论
 *  - 用户系统：扫码登录、Cookie 持久化、用户信息
 *  - 试听检测：freeTrialInfo + 全 quality 探测
 *  - 更新系统：GitHub Releases 检测、安装包/补丁下载
 *  - 天气电台：基于天气生成推荐歌单
 *  - 节拍分析：播客 DJ 节拍检测
...
const crypto = require('crypto');   // 加密（SHA256/SHA512 校验）
const tls = require('tls');         // TLS 证书管理
...
const APP_VERSION = process.env.BHANDSMUSIC_VERSION || APP_PACKAGE.version || '0.9.11';  // 当前版本
const UPDATE_CONFIG = readUpdateConfig(APP_PACKAGE);  // 更新配置
...
/* ==================== 天气服务配置 ==================== */
const OPEN_METEO_FORECAST_URL = 'https://api.open-meteo.com/v1/forecast';    // Open-Meteo 天气 API
const OPEN_METEO_GEOCODE_URL = 'https://geocoding-api.open-meteo.com/v1/search';  // 地理编码 API
const WEATHER_IP_LOCATION_URL = 'http://ip-api.com/json/';  // IP 定位 API
/* 默认位置：上海 */
 ...
 * （播放主流程 / 静默预解析 / 手动重试 / 节拍预取 / TRIAL 兜底，都在 public/js/app/*.js），
 * 而每条路径内部还会按音质档位**降级重试**
...
function readUpdateConfig(pkg) {
  const local = (pkg && pkg.bhandsmusic && pkg.bhandsmusic.update) || {};
  const repoHint = process.env.BHANDSMUSIC_UPDATE_REPOSITORY
  ...
  const owner = process.env.BHANDSMUSIC_UPDATE_OWNER || local.owner || parsed.owner || '';
  const repo = process.env.BHANDSMUSIC_UPDATE_REPO || local.repo || parsed.repo || '';
  return {
    ...
    manifest: process.env.BHANDSMUSIC_UPDATE_MANIFEST
      || process.env.BHANDSMUSIC_UPDATE_MANIFEST_URL
      || process.env.BHANDSMUSIC_UPDATE_MANIFEST_FILE
      || '',
  };
}
...
function readUpdateMirrors(local) {
  const envMirrors = process.env.BHANDSMUSIC_UPDATE_MIRRORS || process.env.BHANDSMUSIC_UPDATE_MIRROR || '';
...
function normalizeManifestUpdateInfo(data) {
  ...
  const patchInfo = patch && patch.downloadUrl ? {
    name: patch.name || updateAssetNameFromUrl(patch.downloadUrl) || `BhandsMusic-${APP_VERSION}→${latestVersion}.patch.json`,
  ...
  const assetInfo = downloadUrl ? {
    name: asset.name || updateAssetNameFromUrl(downloadUrl) || `BhandsMusic-${latestVersion}-Setup.exe`,
  ...
  return {
    ...
    release: {
      ...
      name: release.name || data.name || ('BhandsMusic v' + latestVersion),
...
function localUpdateFallback(reason, opts) {
  ...
  return {
    ...
    release: {
      ...
      name: 'BhandsMusic v' + APP_VERSION,
      version: APP_VERSION,
      htmlUrl: '',
      downloadUrl: '',
...
function parseLatestYmlUpdateInfo(text, reason) {
  ...
  const assetPath = yamlScalar(text, 'path') || yamlScalar(text, 'url') || `BhandsMusic-${latestVersion}-Setup.exe`;
  ...
  try {
    ...
    return {
      ...
      release: {
        ...
        name: data.name || ('BhandsMusic v' + latestVersion),
        version: latestVersion,
...
function safeUpdateFileName(name, version) {
  const raw = String(name || '').trim() || `BhandsMusic-${version || APP_VERSION}.exe`;
  const cleaned = raw
    .replace(/[<>:"/\\|?*\x00-\x1F]/g, '-')
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, 160);
  return cleaned || `BhandsMusic-${version || APP_VERSION}.exe`;
}
...
function writePatchFile(job, file) {
  ...
  const tmp = target + '.bhandsmusic-patch';
...
function normalizePatchPayload(payload) {
  ...
  if (type && type !== 'bhandsmusic-resource-patch') throw new Error('UNSUPPORTED_PATCH_TYPE');
...
/* ====================================================================
 *  HTTP Server - API 路由处理
 *
 *  主要 API 端点：
 *  /api/app/version     - 应用版本信息
 *  /api/update/latest   - 检查更新
 *  /api/update/download - 下载更新
 *  /api/search          - 搜索歌曲
 *  /api/song/url        - 获取歌曲播放地址
 *  /api/lyric           - 获取歌词
 *  /api/login/*         - 登录相关
 *  /api/qq/*            - QQ音乐相关
 *  /api/weather/*       - 天气电台
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/api/app/version') {
    sendJSON(res, {
      name: APP_PACKAGE.name || 'bhandsmusic',
      productName: APP_PACKAGE.productName || 'BhandsMusic',
      version: APP_VERSION,
      update: {
  ...
  // GET /api/kugou/login/qr/check?key=xxx - 轮询扫码状态（回放会话 cookie）；status=4 时保存登录凭据
  if (pn === '/api/kugou/login/qr/check' && req.method === 'GET') {
    try {
      ...
      const session = kugouQrSessions.get(key);
      const cookieHeader = session ? session.cookies.map((c) => c.split(';')[0]).join('; ') : '';
      const meta = await kugouService.apiGetWithMeta('/login/qr/check?key=' + encodeURIComponent(key), 10000, cookieHeader ? { Cookie: cookieHeader } : {});
      const j = meta.json;
      ...
      if (status === 4) {
        ...
        const allCookies = (session ? session.cookies : []).concat(meta.setCookies || []);
        const pairs = {};
        ...
        kugouQrSessions.delete(key);
        console.log('[KugouLogin] 扫码登录成功，凭据已保存（' + cookieStr.split(';').length + ' 项）');
