...
/**
 * 音源时长/类型探测（移植自 Bhands_Web apps/server durationProbe.ts）
 * 只取容器头部/尾部少量字节，解析出音频总时长与容器类型。
 *
 * 用途：第三方音源（GDMusic / kugou / Unblock）都是**按歌名+歌手**匹配的，
 * 同名翻唱、伴奏、串烧、现场版极其常见，匹配错版本时用户会听到「另一首歌」
 * （歌词对不上）。时长是唯一能拿到的、且与版本强相关的客观特征。
 *
 * 另一类必须拦下的情况：**返回的根本不是音频**（HTML 404 / JSON 报错），
 ...
 *  无条件 arrayBuffer() 会把它全读进内存，被候选遍历放大成内存 DoS */
const MAX_PROBE_BYTES = 2 * 1024 * 1024;
const UA = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36';

async function fetchRange(url, range) {
  const ac = new AbortController();
  const timer = setTimeout(function () { ac.abort(); }, PROBE_TIMEOUT_MS);
  try {
    ...
    return { buf: buf, total: Number.isFinite(total) ? total : 0 };
  } catch (e) {
    return null;
  } finally {
    clearTimeout(timer);
...
function findTag(buf, tag, from) {
  return buf.indexOf(Buffer.from(tag, 'latin1'), from || 0);
}
 ...
 * 写死 `frames*1152/44100` 遇到 48kHz 文件会高估 8.8%，
 * 从而把**完全正确的音源**误判为错版本丢弃。
 * 每帧样本数：Layer1=384，Layer2=1152，Layer3 在 MPEG1=1152、MPEG2/2.5=576。
 * Xing/Info 必须从帧头之后开始找，避免命中 ID3 元数据（封面二进制）里的同名串。
 */
function mp3Duration(head, totalBytes) {
  ...
  // VBR：Xing/Info 头带总帧数
  const tags = ['Xing', 'Info'];
  for (let t = 0; t < tags.length; t++) {
    const i = findTag(head, tags[t], p);
...
function oggDuration(head, tail) {
 ...
 */
async function probeAudio(upstreamUrl) {
  ...
  if (kind === 'flac') {
    ...
    durationSec = oggDuration(head.buf, tail ? tail.buf : null);
  } else if (kind === 'mp4') {
  ...
  if (durationSec != null && durationSec > 0) {
    return { status: 'ok', kind: kind, durationSec: durationSec, totalBytes: head.total };
  }
  return { status: 'audio', kind: kind, durationSec: null, totalBytes: head.total };
}
 ...
 *  - 碎片/广告垫片：期望 ≥90s 的歌候选总大小 <400KB → 按绝对大小拒绝
 *    （kuwo 免费试听垫片 11s/185KB 曾靠低码率"伪时长"骗过容差校验）
 *  - 时长可见但与期望差异过大 → 拒绝
 */
function acceptProbe(probe, expectedMs) {
  if (probe.status === 'not-audio') return false;
  if (probe.status === 'unreachable') return true;
  if (expectedMs >= 90000 && probe.totalBytes > 0 && probe.totalBytes < 400000) return false;
  if (probe.status === 'audio') return true;
